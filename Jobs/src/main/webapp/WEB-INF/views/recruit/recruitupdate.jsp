			...
			<c:if test="${loginUser.state !='3'}">
				<div class="col-md-10 col-md-offset-1">
					<form role="form" name="frm" method="post" enctype="multipart/form-data">
						<div class="row">
							<div class="col-md-8">
								<div class="form-group">
									<label for="name">기업 이름</label>
									<input type="text" class="form-control" name="name" id="name" value="${loginUser.name}" readonly="readonly">
								</div>
								<div class="form-group">
									<label for="type">
									공개채용과 특별채용중 선택하시기 바랍니다.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									</label>
									<c:if test="${recruitinfoArr.type ne '1'}">
										<input type="radio" name="type" value="1" id="type">공채
							</c:if>
									<c:if test="${recruitinfoArr.type eq '1'}">
										<input type="radio" name="type" value="1" id="type" checked="checked">공채
							</c:if>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<c:if test="${recruitinfoArr.type ne '2'}">
										<input type="radio" name="type" value="2" id="type">특채
							</c:if>
									<c:if test="${recruitinfoArr.type eq '2'}">
										<input type="radio" name="type" value="2" id="type" checked="checked">특채
							</c:if>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<c:if test="${recruitinfoArr.type ne '3'}">
										<input type="radio" name="type" value="3" id="type">해당없음
							</c:if>
									<c:if test="${recruitinfoArr.type eq '3'}">
										<input type="radio" name="type" value="3" id="type" checked="checked">해당없음
							</c:if>
								</div>
								<div class="form-group">
									<label for="career">신입직원과 경력직원중 선택하시기 바랍니다.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
									<c:if test="${recruitinfoArr.career ne '1'}">
										<input type="radio" name="career" value="1" id="type">신입
							</c:if>
									<c:if test="${recruitinfoArr.career eq '1'}">
										<input type="radio" name="career" value="1" id="type" checked="checked">신입
							</c:if>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<c:if test="${recruitinfoArr.career ne '2'}">
										<input type="radio" name="career" value="2" id="type">경력
							</c:if>
									<c:if test="${recruitinfoArr.career eq '2'}">
										<input type="radio" name="career" value="2" id="type" checked="checked">경력
							</c:if>

									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<c:if test="${recruitinfoArr.career ne '3'}">
										<input type="radio" name="career" value="3" id="type">해당없음
							</c:if>
									<c:if test="${recruitinfoArr.career eq '3'}">
						...
						<div class="form-group">
							<label for="subject">제목</label> <input type="text" class="form-control" name="subject" id="subject" placeholder="제목을 100자 이내로 입력하세요" value="${recruitinfoArr.subject}" onkeyup="test(this.value)">
						</div>
						<div class="text-right text-danger" id="check"></div>
						<div class="form-group">
							<label for="content">내용</label>
							<textarea class="form-control" rows="10" name="content" id="content" placeholder="내용을 2000자 이내로 입력하세요" onkeyup="test2(this.value)">${recruitinfoArr.content}</textarea>
						</div>
						<div class="text-right text-danger" id="check2"></div>
						<div class="form-group">
							<input type="hidden" name="oldFilename" value="${recruitinfoArr.filename}"> 
								<label for="recruitfile">파일 첨부 [ ${recruitinfoArr.viewFname} (${recruitinfoArr.filesize} bytes) ]</label> 
								<input class="form-control-file border" type="file" id="recruitfile" name="recruitfile"> 
...
<script type="text/javascript">
	var check = function() {
		if (!frm.name.value) {
			...
			swal('','신입과 경력중 선택하시기 바랍니다.','warning');
		...
		frm.method = "POST";
		frm.action="recruitupdatecomplete";
		frm.submit();
